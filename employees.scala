...
object employees extends App
{
  ...
  def new_employee(salary_list: List[Int]): List[Int] = {
    var i = 0
    var s_l: List[Int] = salary_list.sorted
    for (salary <- s_l)
    {
  ...
  middle_salary(new_employee(salary_department))
  ...
  def salaryIndexing(salary_list: List[Int], ind: Float = 7)
  {
    for (salary <- salary_list) {
      val floatSalary = salary + (salary * (ind / 100))
      update_salary_staff = update_salary_staff :+ floatSalary.toInt
    }
    println(update_salary_staff)
  }
  ...
  def market_average(worker_level_salary: List[Int], level: String): List[Int] =
  {
    println(s"Введите среднюю зарплату сотрудника уровня '$level': ")

    val salary_stuff = StdIn.readFloat()
    var deviation = List[Float]()

    for (salary <- worker_level_salary)
    {
      deviation = deviation :+ ((salary_stuff - salary.toFloat) / salary.toFloat) * 100
    }
    val mean_percent = deviation.sum / deviation.length
    println(s"Средний процент отклонения зарплаты сотрудника уровня '$level' = $mean_percent")

    var result = List[Int]()
    val length = worker_level_salary.length
    var i = 0
    while (i < length)
    {
      if (deviation(i) > 0)
      {
        result = result :+ (((worker_level_salary(i) / 100) * mean_percent) + worker_level_salary(i)).toInt
      }
      else {
        result = result :+ worker_level_salary(i)
      }
      i += 1
    }
    result
  ...
  def powerOfTwo(num: Int = 2): BigInt =
  {
    var result = 0
    if (num > 1)
    {
  ...
  def powerOfTwoTail(num: Int): BigInt=
  {
    @tailrec
